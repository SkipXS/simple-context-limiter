...
|---|---|---|
| `run` | Running shell commands when full stdout is not needed | `bash`, terminal, `tail -10000 log.txt` |
| `logs` | Extracting relevant errors from tests, builds, lints, and logs | raw test/build output, full server logs |
| `read` | Reading one or more local UTF-8 text files safely | `cat huge.log`, `type huge.log`, repeated file reads |
| `search` | Searching local files with bounded ripgrep output or optional ast-grep structural search | raw `rg` / `grep` / `sg` commands |
| `discover` | Repo summaries, file lists, trees, and source outlines | broad globs, recursive trees, several setup reads |
| `fetch` | Fetching web pages as readable text | `webfetch`, raw HTML downloads |
...
Commands that exit successfully but write diagnostics to stderr will not include stderr in `run`; use `logs` when stderr or mixed command output matters.
...
Use `logs` for test/check commands when you want error blocks or a compact tail fallback instead of full output.
...
Use `usage` to see aggregate savings stats, local usage reports, or guidance from local usage patterns:
...
`diff` only reports tracked working-tree or staged changes covered by `git diff`. It does not include untracked files unless they have been staged. In `mode: "status"`, `staged: false` shows only unstaged tracked changes and `staged: true` shows only staged tracked changes.
...
`SIMPLE_CONTEXT_LIMITER_SHELL` is optional. Set it when you want `run` to use the same shell style as OpenCode, for example Git for Windows `bash`. Without it, Node uses the platform default shell (`cmd.exe` on Windows).
...
If you need a specific command shell for `run`, set `SIMPLE_CONTEXT_LIMITER_SHELL` in the environment that starts Claude Code.
...
|---|---:|---|
| `SIMPLE_CONTEXT_LIMITER_SHELL` | Node platform default | Shell used by `run` |
| `SIMPLE_CONTEXT_LIMITER_RG_PATH` | auto-detect | Explicit path to `rg` / `rg.exe` for `search` |
| `SIMPLE_CONTEXT_LIMITER_AST_GREP_PATH` | auto-detect | Explicit path to `sg` / `ast-grep` for `search` with `engine: "ast"` |
...
`fetch` caches fetched content for 1 hour in `~/.simple-context-limiter/cache.json` and prunes old entries on load/save. The cache is capped by entry count and total content bytes. Delete that file anytime to clear the cache.

`usage` with `mode: "report"` reads `~/.simple-context-limiter/usage.jsonl`. Delete that file anytime to clear collected usage metadata.
